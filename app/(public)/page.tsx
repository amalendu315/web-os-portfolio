...
import {
    ...
    FileText, Activity, Lock, Briefcase, CheckCircle2, Server, ShieldCheck,
    Wrench, Mail, Send, Calculator, Volume2, Dna, BrainCircuit, Rocket,
    Settings, Monitor, Command, RefreshCw, ArrowUp, Home, ChevronLeft, Square, Signal
} from 'lucide-react';
...
const Clock: React.FC<{ mobile?: boolean }> = ({ mobile }) => {
    const [time, setTime] = useState<Date | null>(null);
    ...
    if (mobile) {
        return (
            <span className="text-xs font-bold text-slate-100 tracking-widest">
...
// 2. LOGIN SCREEN
const LoginScreen: React.FC<{ onLogin: () => void }> = ({ onLogin }) => (
    <div className="fixed inset-0 bg-slate-950 flex items-center justify-center z-[90] font-mono bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-slate-800 to-slate-950 p-4">
        <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-700/50 p-6 md:p-10 rounded-2xl shadow-2xl w-full max-w-sm md:max-w-md text-center relative overflow-hidden group">
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-32 h-32 bg-green-500/20 blur-[50px] rounded-full"></div>
            <div className="w-24 h-24 bg-slate-800/80 rounded-full mx-auto mb-6 flex items-center justify-center border border-slate-600 shadow-inner relative z-10">
                <User size={40} className="text-slate-200" />
                <div className="absolute bottom-0 right-0 bg-green-500 rounded-full p-1.5 border-4 border-slate-900">
            ...
            <h2 className="text-xl md:text-2xl text-white mb-1 font-bold tracking-wider">AMALENDU PANDEY</h2>
            <p className="text-green-400/80 mb-8 text-[10px] md:text-xs tracking-widest uppercase">Cyber Security & Full Stack Engineer</p>
            <div className="space-y-4 relative z-10">
                <div className="relative group/input">
                    <Lock size={14} className="absolute left-3 top-3 text-slate-500" />
                    <input
                        type="password"
                        placeholder="Enter Bio-Key"
                        className="w-full bg-slate-950/50 border border-slate-700 rounded-lg pl-10 pr-4 py-2.5 text-white focus:outline-none cursor-not-allowed opacity-70 text-sm"
                        disabled
                    />
                </div>
                <button
                    onClick={onLogin}
                    className="w-full bg-green-600 hover:bg-green-500 hover:shadow-[0_0_20px_rgba(34,197,94,0.3)] text-black font-bold py-2.5 rounded-lg transition-all duration-300 flex items-center justify-center gap-2 transform active:scale-[0.98] text-sm md:text-base"
                >
                    <Power size={18} /> INITIALIZE GUEST SESSION
                </button>
            </div>
...
const WindowFrame: React.FC<WindowFrameProps> = ({ window, activeId, onFocus, onClose, onMinimize, onMaximize, children }) => {
    ...
    return (
        <motion.div
            drag={!isMaximized} // Disable drag on desktop max, and via CSS pointer-events on mobile
            dragMomentum={false}
            dragConstraints={{ left: 0, top: 0, right: 100, bottom: 100 }} // Simplified constraints
            initial="initial"
            animate="animate"
            exit="exit"
            variants={variants}
            transition={{ type: "spring", damping: 25, stiffness: 300 }}
            // Mobile: fixed inset, no round, high z-index. Desktop: standard window
            className={`absolute flex flex-col bg-slate-900/95 md:bg-slate-900/90 backdrop-blur-xl border shadow-2xl overflow-hidden
                ${isFocused ? 'border-slate-500 ring-1 ring-slate-700/50 z-50' : 'border-slate-800 z-0'}
                ${!isMaximized ? 'md:top-20 md:left-[15%]' : ''}
                
                /* MOBILE OVERRIDES (Make it look like an App) */
                max-md:!top-[30px] max-md:!left-0 max-md:!w-full max-md:!h-[calc(100%-80px)] max-md:!rounded-none max-md:!border-none max-md:!transform-none
            `}
            style={{ zIndex: window.zIndex }}
            onMouseDown={() => onFocus(window.id)}
        >
            {/* Window Header - Hidden on Mobile to look like native app, or styled differently */}
            <div
                onDoubleClick={() => onMaximize(window.id)}
                className="h-10 bg-slate-800/80 border-b border-slate-700 flex items-center justify-between px-3 cursor-default select-none shrink-0"
            >
                ...
                {/* Mobile Close Button (Just in case) */}
                <button onClick={(e) => { e.stopPropagation(); onClose(window.id); }} className="md:hidden p-1.5 active:bg-slate-700 rounded text-slate-400"><X size={18}/></button>
            </div>
...
const SettingsApp: React.FC<{ currentBg: string; onBgChange: (id: string) => void; isGodMode: boolean }> = ({ currentBg, onBgChange, isGodMode }) => {
    ...
    return (
        <div className="h-full bg-slate-950 text-slate-200 p-4 md:p-6">
            <h1 className="text-2xl font-bold mb-6 flex items-center gap-2">
                <Settings size={24} className="text-slate-400" /> System Settings
            </h1>
            <div className="space-y-8">
                ...
                <div>
                    ...
                    <div className="bg-slate-900 rounded-lg border border-slate-800 p-4 space-y-2 font-mono text-xs md:text-sm">
                        <div className="flex justify-between"><span>User Level:</span> <span className={isGodMode ? "text-yellow-400 font-bold" : "text-slate-400"}>{isGodMode ? "GOD MODE" : "GUEST"}</span></div>
                        <div className="flex justify-between"><span>OS Version:</span> <span className="text-slate-400">AP-OS Mobile v4.1</span></div>
                    </div>
                </div>
            </div>
        </div>
...
const CommandPalette: React.FC<{ isOpen: boolean; onClose: () => void; apps: AppRegistry; onOpenApp: (key: string) => void }> = ({ isOpen, onClose, apps, onOpenApp }) => {
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-start justify-center pt-[10vh] md:pt-[20vh] bg-black/50 backdrop-blur-sm" onClick={onClose}>
            <div className="w-full max-w-sm md:max-w-xl bg-slate-900 border border-slate-700 rounded-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 mx-4" onClick={e => e.stopPropagation()}>
                <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-800">
                    <Search size={20} className="text-slate-500" />
                    <input
                        ref={inputRef}
...
const TextEditorApp: React.FC<{ file: FileSystemItem | null }> = ({ file }) => (
    <div className="h-full flex flex-col bg-slate-950 text-slate-300 font-mono text-sm">
        ...
        <div className="flex-1 p-4 overflow-auto">
            <pre className="outline-none w-full h-full font-mono leading-relaxed text-xs md:text-sm">
                {file ? <code className="language-typescript">{file.content}</code> : <span className="text-slate-600">// Select a file from Explorer</span>}
            </pre>
        </div>
    </div>
...
const TerminalApp: React.FC = () => {
    const [history, setHistory] = useState<{ type: string; content: string }[]>([
        { type: 'output', content: 'AP-OS Mobile Kernel v4.1 initialized.' },
        { type: 'output', content: 'Connected to MongoDB Atlas.' }
    ]);
    const [input, setInput] = useState('');
    const bottomRef = useRef<HTMLDivElement>(null);
    ...
    const handleCommand = (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === 'Enter') {
            ...
            switch (cmd) {
                case 'help': response = 'Available: whoami, clear, date, reboot'; break;
                case 'whoami': response = 'guest@portfolio-mobile'; break;
                case 'clear': setHistory([]); return;
                case 'date': response = new Date().toString(); break;
                default: response = `Command not found: ${cmd}`;
            }
    ...
    return (
        <div className="p-4 font-mono text-xs md:text-sm bg-black/90 h-full text-green-500 selection:bg-green-500/30 overflow-y-auto">
            {history.map((line, i) => (
                <div key={i} className={`mb-1 break-all ${line.type === 'input' ? 'text-white mt-3' : 'text-green-500/80'}`}>
                    {line.type === 'input' ? '> ' : ''}{line.content}
                </div>
            ))}
            <div className="flex mt-2">
...
const ExplorerApp: React.FC<{ onOpenFile: (file: FileSystemItem) => void; fileSystem: FileSystemItem[] }> = ({ onOpenFile, fileSystem }) => {
    ...
    const handleBack = () => { if (historyIndex > 0) { setHistoryIndex(historyIndex - 1); setCurrentPath(history[historyIndex - 1]); }};
    const handleUp = () => {
        const currentFolder = fileSystem.find(f => f._id === currentPath);
        if (currentFolder && currentFolder.parentId) navigateTo(currentFolder.parentId);
    };

    const currentFolder = fileSystem.find(f => f._id === currentPath);
    const currentFiles = fileSystem.filter(f => f.parentId === currentPath || (currentFolder && f.parentId === currentFolder.name));

    return (
        <div className="flex h-full flex-col bg-slate-900 text-slate-200 font-sans">
            <div className="flex items-center gap-2 p-2 border-b border-slate-700 bg-slate-800/80 backdrop-blur-md">
                <div className="flex items-center gap-1">
                    <button onClick={handleBack} disabled={historyIndex === 0} className="p-1.5 rounded hover:bg-slate-700 disabled:opacity-30"><ChevronLeft size={18} /></button>
                    <button onClick={handleUp} disabled={currentPath === 'root'} className="p-1.5 rounded hover:bg-slate-700 disabled:opacity-30"><ArrowUp size={18} /></button>
                </div>
                <div className="flex-1 bg-slate-950 border border-slate-700 rounded px-2 py-1 text-xs truncate">~/home/{currentFolder?.name || 'root'}</div>
            </div>
            <div className="flex-1 p-4 bg-slate-900/50 overflow-y-auto">
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4 content-start">
                    {currentFiles.length === 0 && <div className="col-span-full text-center text-slate-500 text-sm mt-10">Empty Folder</div>}
                    {currentFiles.map(file => (
                        <div key={file._id} onClick={() => file.type === 'folder' ? navigateTo(file._id) : onOpenFile(file)} className="flex flex-col items-center gap-2 p-3 bg-white/5 rounded-lg border border-white/5 active:bg-white/10 transition-all cursor-pointer">
                            <div className={`${file.type === 'folder' ? 'text-yellow-500' : 'text-blue-400'}`}>
                                {file.type === 'folder' ? <Folder fill="currentColor" size={40} /> : <FileText size={36} />}
                            </div>
                            <span className="text-xs text-center w-full font-medium text-slate-200 truncate">{file.name}</span>
                        </div>
                    ))}
                </div>
            </div>
...
const QuestLogApp: React.FC<{ quests: any[] }> = ({ quests }) => {
    const [openItem, setOpenItem] = useState<string | null>(null);
    return (
        <div className="h-full w-full bg-slate-950 text-slate-200 overflow-y-auto p-4 md:p-8">
            <h1 className="text-2xl md:text-3xl font-bold font-sans text-green-400 mb-2">Quest Log</h1>
            <div className="w-full space-y-3">
                {quests.map((quest, index) => {
                    const isOpen = openItem === `item-${index}`;
                    return (
                        <div key={index} className="border border-slate-800 bg-slate-900/40 rounded-lg overflow-hidden">
                            <button onClick={() => setOpenItem(isOpen ? null : `item-${index}`)} className="w-full flex items-center justify-between p-4 hover:bg-slate-800/30 text-left">
                                <div className="flex items-center gap-3">
                                    <div className="p-2 rounded bg-green-500/10 text-green-500"><Server size={16} /></div>
                                    <div><span className="font-bold text-sm block">{quest.title}</span><span className="text-[10px] text-slate-500 uppercase">{quest.status}</span></div>
                                </div>
                                <ChevronRight size={16} className={`text-slate-500 transition-transform ${isOpen ? 'rotate-90' : ''}`} />
                            </button>
                            <AnimatePresence>
                                {isOpen && (
                                    <motion.div initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }} className="overflow-hidden bg-slate-950/30">
                                        <div className="p-4 pt-0 text-sm text-slate-400">
                                            <p className="mb-3">{quest.description}</p>
                                            <ul className="space-y-2">{quest.tasks?.map((t: string, i: number) => <li key={i} className="flex gap-2"><CheckCircle2 size={14} className="mt-0.5 text-green-500"/> {t}</li>)}</ul>
                                        </div>
                                    </motion.div>
                                )}
                            </AnimatePresence>
...
// 10. APP: WORK EXPERIENCE
const WorkExperienceApp: React.FC<{ experience: any[] }> = ({ experience }) => (
    <div className="p-4 md:p-6 bg-slate-950 text-slate-300 h-full font-sans overflow-y-auto">
        <h2 className="text-xl font-bold text-blue-400 mb-6 flex items-center gap-2"><Briefcase size={20} /> Career Timeline</h2>
        <div className="border-l-2 border-slate-800 pl-4 space-y-8 ml-2">
            {experience.map((item, i) => (
                <div key={i} className="relative">
                    <div className={`absolute -left-[25px] top-1 bg-${item.color}-500 h-4 w-4 rounded-full border-4 border-slate-950`}></div>
                    <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-800">
                        <div className="flex flex-col md:flex-row md:justify-between md:items-start mb-2">
                            <h3 className="text-base font-bold text-slate-100">{item.title}</h3>
                            <span className="text-xs font-mono text-slate-500">{item.date}</span>
                        </div>
                        <p className="text-xs text-blue-400 mb-2 font-mono">{item.sub}</p>
...
// 11. APP: ABOUT ME
const AboutApp: React.FC<{ profile: any }> = ({ profile }) => (
    <div className="h-full w-full bg-slate-950 text-slate-200 overflow-y-auto p-4">
        <div className="flex flex-col items-center max-w-xl mx-auto">
            <div className="h-20 w-20 rounded-full border-2 border-green-500 bg-slate-900 flex items-center justify-center mb-4 shadow-[0_0_20px_rgba(34,197,94,0.3)]">
                <User size={40} className="text-slate-200" />
            </div>
            <h1 className="text-2xl font-bold text-white mb-1">{profile.name}</h1>
            <p className="text-center text-xs text-green-400 font-mono mb-6 bg-green-500/10 px-3 py-1 rounded-full border border-green-500/20">{profile.profile}</p>
            <div className="w-full space-y-4">
                <div className="bg-slate-900/30 p-4 rounded-lg border border-slate-800">
                    <h2 className="flex items-center gap-2 text-sm font-semibold text-green-400 mb-3"><Dna size={16}/> Skills</h2>
                    <div className="flex flex-wrap gap-2">
                        {profile.keySkills && profile.keySkills.map((skill: string) => (
                            <span key={skill} className="px-2 py-1 rounded bg-slate-800 border border-slate-700 text-[10px] text-slate-300">{skill}</span>
                        ))}
                    </div>
                </div>
                <div className="bg-slate-900/30 p-4 rounded-lg border border-slate-800">
                    <h2 className="flex items-center gap-2 text-sm font-semibold text-purple-400 mb-2"><Rocket size={16}/> Bio</h2>
...
export default function WebOS() {
        ...
        if (bootState === 'desktop') {
            const fetchData = async () => {
                try {
                    ...
                    const files = await filesRes.json();
                    setFileSystem(files.length > 0 ? files : [{ _id: 'root', name: 'Root', type: 'folder', parentId: null }]);
                    setQuests(await questsRes.json());
                    setExperience(await expRes.json());
                    setProfile(await profRes.json());
    ...
    const APPS: AppRegistry = {
        terminal: { title: 'Terminal', icon: <Terminal size={16} className="text-green-400" />, component: TerminalApp },
        explorer: { title: 'Explorer', icon: <HardDrive size={16} className="text-yellow-400" />, component: ExplorerApp },
        editor: { title: 'Editor', icon: <Code size={16} className="text-blue-400" />, component: TextEditorApp },
        quest: { title: 'Quests', icon: <List size={16} className="text-purple-400" />, component: QuestLogApp },
        experience: { title: 'Work', icon: <Briefcase size={16} className="text-orange-400" />, component: WorkExperienceApp },
        settings: { title: 'Settings', icon: <Settings size={16} className="text-slate-300" />, component: SettingsApp },
        about: { title: 'Me', icon: <User size={16} className="text-pink-400" />, component: AboutApp }
    };
    ...
    const openWindow = (appId: string, props: any = {}) => {
        ...
        const newWindow: WindowInstance = {
            id, appId, title: props.file ? props.file.name : APPS[appId].title,
            icon: APPS[appId].icon,
            isOpen: true, isMinimized: false, isMaximized: false, zIndex: maxZ + 1,
            props: { ...props, currentBg: bgStyle, onBgChange: setBgStyle, fileSystem, quests, experience, profile },
        };
        setWindows([...windows, newWindow]);
        setActiveWindowId(id);
    };

    const closeWindow = (id: number) => { setWindows(prev => prev.filter(w => w.id !== id)); setActiveWindowId(null); };
    const closeActiveWindow = () => { if(activeWindowId) closeWindow(activeWindowId); };
    const minimizeAll = () => setActiveWindowId(null);

    // Keyboard (Ctrl+K)
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            if ((e.ctrlKey || e.metaKey) && e.key === 'k') { e.preventDefault(); setPaletteOpen(prev => !prev); }
        };
        window.addEventListener('keydown', handleKeyDown);
        return () => window.removeEventListener('keydown', handleKeyDown);
    }, []);
    ...
    return (
        <div className="fixed inset-0 bg-slate-950 overflow-hidden font-sans select-none text-slate-200">
            ...
            <div className="md:hidden absolute top-0 w-full h-8 flex items-center justify-between px-4 z-50 bg-black/20 backdrop-blur-sm text-xs font-medium text-white">
                <Clock mobile />
                <div className="flex items-center gap-2">
                    <Signal size={12} />
                    <Wifi size={12} />
            ...
            <div className="absolute top-8 bottom-12 left-0 right-0 p-4 z-10 overflow-y-auto">
                <div className="grid grid-cols-4 gap-4 md:flex md:flex-col md:flex-wrap md:max-h-[90vh] md:content-start md:gap-4 md:w-auto">
                    {Object.keys(APPS).map(key => (
            ...
            <AnimatePresence>
                {windows.map(win => {
                    const Component = APPS[win.appId].component;
                    if (win.isMinimized && activeWindowId !== win.id) return null; // Logic for desktop minimize
                    // On mobile, we only show the active window effectively full screen
            ...
            <div className="md:hidden absolute bottom-0 w-full h-12 bg-black/80 backdrop-blur-md border-t border-slate-800 z-[100] flex justify-around items-center text-slate-400">
                <button onClick={closeActiveWindow} className="p-4 active:text-white"><ChevronLeft size={24} /></button>
                <button onClick={minimizeAll} className="p-4 active:text-white"><div className="w-4 h-4 rounded-full border-2 border-current"></div></button>
                <button onClick={() => setPaletteOpen(true)} className="p-4 active:text-white"><Square size={18} /></button>
            </div>

            {/* --- DESKTOP TASKBAR --- */}
            <div className="hidden md:flex absolute bottom-0 w-full h-12 bg-slate-900/90 backdrop-blur-md border-t border-slate-700 z-[100] items-center px-4 justify-between shadow-2xl">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex gap-2">
                        {windows.map(win => (
                            <button key={win.id} onClick={() => setActiveWindowId(win.id)} className={`flex items-center gap-2 px-3 py-1.5 text-xs rounded border transition-all max-w-[140px] ${activeWindowId === win.id && !win.isMinimized ? 'bg-slate-800 border-slate-600 text-green-400' : 'border-transparent text-slate-400 hover:bg-slate-800/50'}`}>
                                {React.isValidElement(win.icon) ? win.icon : null} <span className="truncate">{win.title}</span>
                            </button>
                        ))}
                    </div>
                ...
                <div className="flex items-center gap-4">
                    <div className="hidden md:flex items-center gap-2 px-2 py-1 bg-slate-800/50 rounded border border-slate-700/50 text-[10px] text-slate-400 font-mono"><Command size={10} /> <span>CTRL+K</span></div>
                    <Volume2 size={16} className="text-slate-400 hover:text-white cursor-pointer" />
